...
from safe_rl_stable_baselines3.common.buffers import (
    DictRolloutBuffer,
    RolloutBuffer,
    ConstraintRolloutBuffer,
)
...
from safe_rl_stable_baselines3.common.utils import obs_as_tensor, safe_mean
...
class RewardConstrainedOnPolicyAlgorithm(OnPolicyAlgorithm):
    """
    The base for On-Policy algorithms (ex: A2C/PPO).
    ...
    def __init__(
        ...
        from safe_rl_stable_baselines3.common import vec_env
    ...
    def collect_rollouts(
        ...
        env: VecEnv,
        callback: BaseCallback,
        rollout_buffer: ConstraintRolloutBuffer,
        n_rollout_steps: int,
    ) -> bool:
        """
        Collect experiences using the current policy and fill a ``RolloutBuffer``.
        ...
        while n_steps < n_rollout_steps:
            ...
            # get constraint from infos into one array
            constraints = np.array([infos[i]["constraint"] for i in range(len(infos))])
            rollout_buffer.add(
                self._last_obs,
                actions,
                rewards,
