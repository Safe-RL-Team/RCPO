...
from safe_rl_stable_baselines3.common.preprocessing import get_action_dim, get_obs_shape
from safe_rl_stable_baselines3.common.type_aliases import (
    DictReplayBufferSamples,
    DictRolloutBufferSamples,
    ReplayBufferSamples,
    RolloutBufferSamples,
    ConstraintRolloutBufferSamples,
)
from safe_rl_stable_baselines3.common.utils import get_device
from safe_rl_stable_baselines3.common.vec_env import VecNormalize
...
class RolloutBuffer(BaseBuffer):
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.constraints[batch_inds],
        )
        return RolloutBufferSamples(*tuple(map(self.to_torch, data)))


class ConstraintRolloutBuffer(RolloutBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.advantages[batch_inds].flatten(),
            self.returns[batch_inds].flatten(),
            self.constraints[batch_inds].flatten(),
        )
        return ConstraintRolloutBufferSamples(*tuple(map(self.to_torch, data)))
